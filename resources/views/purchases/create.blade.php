                ...
                <form action="{{ route('purchases.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select id="product_id" name="product_id" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" data-price="{{ $product->buy_price }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" id="quantity" name="quantity" min="1" value="{{ old('quantity', 1) }}" required class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @error('quantity')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Unit Price -->
                        <div>
                            <label for="buy_price" class="block text-sm font-medium text-gray-700">Unit Price (MAD) *</label>
                            <input type="number" step="0.01" id="buy_price" name="buy_price" min="0" value="{{ old('buy_price') }}" required class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @error('buy_price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" id="purchase_date" name="purchase_date" value="{{ old('purchase_date', date('Y-m-d')) }}" required class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @error('purchase_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // When product changes, update the buy price
    productSelect.addEventListener('change', function() {
        const selectedOption = this.options[this.selectedIndex];
        if (selectedOption && selectedOption.dataset.price) {
            buyPriceInput.value = selectedOption.dataset.price;
            calculateTotal();
        }
    });
