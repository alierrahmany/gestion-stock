    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-gray-800">
                <i class="fas fa-shopping-cart mr-2 text-blue-500"></i>Modifier la Vente
            </h1>
                ...
                <form action="{{ route('sales.update', $sale->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select id="product_id" name="product_id" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $sale->product_id == $product->id ? 'selected' : '' }} data-price="{{ $product->price }}">
                                        {{ $product->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <p id="stock-info" class="mt-1 text-sm text-gray-600">Stock disponible: <span id="available-stock">{{ $availableStock }}</span></p>
                            <p id="stock-warning" class="mt-1 text-sm text-red-600 hidden">Stock insuffisant pour cette modification!</p>
                            @error('quantity')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // When product changes, update price and stock
    productSelect.addEventListener('change', function() {
        ...
        // Fetch available stock for the new product
        if (this.value) {
