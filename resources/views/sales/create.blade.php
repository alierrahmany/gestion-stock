    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-gray-800">
                <i class="fas fa-shopping-cart mr-2 text-blue-500"></i>New Sale
            </h1>
                ...
                <form action="{{ route('sales.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p id="stock-info" class="mt-1 text-sm text-gray-600">Available stock: <span id="available-stock">0</span></p>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" id="price" name="price" min="0" value="{{ old('price') }}" required class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @error('price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" id="date" name="date" value="{{ old('date', date('Y-m-d')) }}" required class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            @error('date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    productSelect.addEventListener('change', function() {
        if (this.value) {
            fetch(`/sales/stock/${this.value}`)
                ...
                .then(data => {
                    availableStockSpan.textContent = data.stock;
    ...
    // Initialize if returning with old input
    if (productSelect.value) {
        fetch(`/sales/stock/${productSelect.value}`)
            .then(response => response.json())
            .then(data => {
                availableStockSpan.textContent = data.stock;
                stockInfo.classList.remove('hidden');
